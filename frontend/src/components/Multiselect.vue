...
export default {
  ...
  methods: {
    ...
    async builder() {
                ...
                for (let k = 0; k < response.data.length; ++k) {
                    ...
                    for (let j = 0; j < response.data.length; ++j) {
                        if (response.data[k].name.pl == response.data[j].name.pl) {
                            ...
                            let time = new Date(response.data[j].start_time).getTime();
                            if (beg_time < time) {
                                beg_time = time
                                beg_date = response.data[j].start_time
                            }
                        }
                    }
                    tmp.push({
        ...
        let shift = beg_date.substring(0, 10)
        var date = new Date();
        ...
        ((date.getDate() <= 9) ? '0' : '') +
        date.getDate()
        console.log("datka " + shift)
        // zapytanie o przesuniętych
        for (let i = 0; i < this.courses_id.length; i++) {
            let tmp = []
            await axios
            .get('https://usosapps.uw.edu.pl/services' + '/tt/course_edition', {
            ...
            .then(response => {
                ...
                // dodawanie do tych z poprzedniego tygodnia
                for (let k = 0; k < tmp.length; ++k) {
                    let flag = false
                    for (let j = 0; j < this.courses_dates[i].dates; ++j) {
                        if (tmp[k].name == this.courses_dates[i][j].name) {
                            for (let l = 0; l < tmp[k].dates.length; ++l) {
                                this.courses_dates[i][j].push(tmp[k].dates[l])
                            }
                            flag = true
                        }
                    }
                    if (flag == false) {
                        this.courses_dates[i].push(tmp[k])
                    }
                }
            })
            .catch(error => {
